...
public class MocapLoopMod implements ModInitializer {
	...
	public void onInitialize() {
		...
		// Register an entity join callback to catch players joining the world.
		// This should fire every time a player entity is added to a world.
		EntityJoinWorldCallback.EVENT.register((World world, Entity entity) -> {
			if (!world.isClient && entity instanceof ServerPlayerEntity) {
				ServerPlayerEntity player = (ServerPlayerEntity) entity;
				System.out.println("DEBUG: EntityJoinWorldCallback: " + player.getName().getString() + " joined world.");
				// Get the server instance from the world.
	...
	private void onPlayerDisconnect(MinecraftServer server, ServerPlayerEntity player) {
		...
		executeCommand(server, "run mocap recording save loop" + getLoopCount(server));
	...
	public void startLoop(MinecraftServer server) {
		...
		scheduleCommand(server, "mocap recording start @a", 300);
